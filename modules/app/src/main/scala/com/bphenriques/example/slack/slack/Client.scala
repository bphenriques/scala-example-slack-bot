...
object Slack {
  ...
  def fromClient(client: AsyncMethodsClient): Slack = new Slack {
    ...
    private def adaptApiError[A <: SlackApiTextResponse](call: IO[A], getError: A => ErrorResponseMetadata): IO[A] =
      call
        .flatMap { response =>
          if (response.isOk) IO.pure(response)
          else IO.raiseError(SlackApiError.from(response, getError(response))) // Weird but that is the behaviour.
        }
